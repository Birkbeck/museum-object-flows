...
RESULTS_DIRECTORY = "scratch/classifications"
...
def run_experiment(
    ...
    building_notes["prompt"] = building_notes.apply(
        lambda row: generate_prompt(
            ROLES[role_name], TASKS[task_name], examples, row["building_notes"]
        ),
        axis=1,
    )
    building_notes["llm_response"] = building_notes.apply(
        lambda row: get_llm_response(MODELS[model_name], row["prompt"], temperature),
        axis=1,
    )
    building_notes["predicted_change_in_status"] = building_notes.apply(
        lambda row: extract_labels(row["llm_response"], "status"),
        axis=1,
    )
    building_notes["predicted_change_in_use"] = building_notes.apply(
        lambda row: extract_labels(row["llm_response"], "use"),
        axis=1,
    )
    building_notes["predicted_change_in_responsibility"] = building_notes.apply(
